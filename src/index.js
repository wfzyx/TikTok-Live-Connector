...
class WebcastPushConnection extends EventEmitter {
		...
		try {
			...
			try {
				...
				try {
					...
					retryAfter = ex.response.headers?.["retry-after"]
						? parseInt(ex.response.headers["retry-after"])
						: null;
				} catch (parseErr) {
					throw ex;
				}
				...
				const errorMessage =
					jsonError?.error || "Failed to retrieve the initial room data.";
			...
			if (!this.#isWsUpgradeDone) {
				if (!this.#options.enableRequestPolling) {
					throw new NoWSUpgradeError(
						"TikTok does not offer a websocket upgrade and request polling is disabled (`enableRequestPolling` option).",
					);
				}
				...
				if (!this.#options.sessionId) {
					...
					throw new NoWSUpgradeError(
						"TikTok does not offer a websocket upgrade. Please provide a valid `sessionId` to use request polling instead.",
					);
				}
	...
	async sendMessage(text, sessionId) {
		...
		if (!this.#options.sessionId) {
			throw new InvalidSessionIdError(
				"Missing SessionId. Please provide your current SessionId to use this feature.",
			);
		}
		...
		try {
			...
			let response = await this.#httpClient.postFormDataToWebcastApi(
				"room/chat/",
				requestParams,
				null,
			);
			...
			switch (response?.status_code) {
				case 20003:
					throw new InvalidSessionIdError(
						"Your SessionId has expired. Please provide a new one.",
					);
				default:
					throw new InvalidResponseError(
						`TikTok responded with status code ${response?.status_code}: ${response?.data?.message}`,
						response,
					);
			}
		} catch (err) {
			throw new InvalidResponseError(
				`Failed to send chat message. ${err.message}`,
				err,
			);
		}
	}
	...
	async decodeProtobufMessage(messageType, messageBuffer) {
		switch (messageType) {
			case "WebcastResponse": {
				let decodedWebcastResponse = deserializeMessage(
					messageType,
					messageBuffer,
				);
			...
			case "WebcastWebsocketMessage": {
				let decodedWebcastWebsocketMessage =
					await deserializeWebsocketMessage(messageBuffer);
				if (
					typeof decodedWebcastWebsocketMessage.webcastResponse === "object"
				) {
					this.#processWebcastResponse(
						decodedWebcastWebsocketMessage.webcastResponse,
					);
				}
				break;
			}
	...
	async #retrieveRoomId() {
		try {
			let mainPageHtml = await this.#httpClient.getMainPage(
				`@${this.#uniqueStreamerId}/live`,
			);
			...
			try {
				...
				if (roomData.statusCode)
					throw new InvalidResponseError(
						`API Error ${roomData.statusCode} (${roomData.message || "Unknown Error"})`,
						undefined,
					);
			...
			throw new ExtractRoomIdError(
				`Failed to retrieve room_id from page source. ${err.message}`,
			);
		}
	}
	...
	async #fetchRoomInfo() {
		try {
			let response = await this.#httpClient.getJsonObjectFromWebcastApi(
				"room/info/",
				this.#clientParams,
			);
			...
			throw new InvalidResponseError(
				`Failed to fetch room info. ${err.message}`,
				err,
			);
		}
	}
	...
	async #fetchAvailableGifts() {
		try {
			...
			throw new InvalidResponseError(
				`Failed to fetch available gifts. ${err.message}`,
				err,
			);
		}
	}
	...
	async #fetchRoomData(isInitial) {
		let webcastResponse =
			await this.#httpClient.getDeserializedObjectFromWebcastApi(
				"im/fetch/",
				this.#clientParams,
				"WebcastResponse",
				isInitial,
		...
		if (webcastResponse.internalExt)
				...
				switch (message.type) {
					...
					case "WebcastGiftMessage":
						...
						if (Array.isArray(this.#availableGifts) && simplifiedObj.giftId) {
							simplifiedObj.extendedGiftInfo = this.#availableGifts.find(
								(x) => x.id === simplifiedObj.giftId,
							);
						}
